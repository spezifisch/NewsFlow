<script lang="ts">
  ...
  import { show as showConfig } from './components/ConfigDialog.svelte';
  import { writable } from 'svelte/store';
  import axios from 'axios';

  let feeds = writable([]);
  let articles = writable([]);
  let selectedFeed = writable(null);
  ...
  async function fetchArtifact() {
    ...
    const runs = await axios.get('https://api.github.com/repos/your-username/your-repo/actions/runs', { headers });
    const runId = runs.data.workflow_runs[0].id;

    const artifacts = await axios.get(`https://api.github.com/repos/your-username/your-repo/actions/runs/${runId}/artifacts`, { headers });
    const artifactId = artifacts.data.artifacts[0].id;

    const download = await axios.get(`https://api.github.com/repos/your-username/your-repo/actions/artifacts/${artifactId}/zip`, { headers, responseType: 'blob' });
...
<main class="flex h-screen">
  <div class="w-1/3 bg-gray-100 p-4">
    <button on:click={() => showConfig.set(true)} class="mb-4 p-2 bg-blue-600 text-white rounded-md">Settings</button>
    <ul>
      {#each $feeds as feed (feed.feed_id)}
        <li on:click={() => selectFeed(feed.feed_id)} class="cursor-pointer p-2 rounded-md hover:bg-gray-200">
          {feed.feed_title}
        </li>
      {/each}
    </ul>
    ...
    <ul>
      {#each $articles.filter(article => $selectedFeed === null || article.feed_id === $selectedFeed) as article (article.title)}
        <li class="p-2 border-b border-gray-200">
          ...
          <a href={article.link} target="_blank" class="text-blue-600">Read more</a>
        </li>
      {/each}
    </ul>
  </div>
